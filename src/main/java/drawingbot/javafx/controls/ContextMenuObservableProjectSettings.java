...
public class ContextMenuObservableProjectSettings extends ContextMenu {
    ...
    public ContextMenuObservableProjectSettings(TableRow<ObservableVersion> row) {
        ...
        menuLoad.setOnAction(e -> Register.PRESET_LOADER_PROJECT.getDefaultManager().applyPreset(DrawingBotV3.context(), row.getItem().getPreset()));
        getItems().add(menuLoad);

        MenuItem menuSave = new MenuItem("Save as project");
        menuSave.setOnAction(e -> FXHelper.exportPreset(row.getItem().getPreset(), new File(row.getItem().getPreset().data.imagePath).getParentFile(), row.getItem().name.get(), false));
