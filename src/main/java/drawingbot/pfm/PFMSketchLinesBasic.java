...
public class PFMSketchLinesBasic extends AbstractSketchPFM {
    ...
    public void nextPathFindingResult(PathFindingContext context, IPixelData pixels) {
        ...
        float startAngle = tools.randomFloat(startAngleMin, startAngleMax);
        int nextLineLength = tools.randomInt(minLineLength, maxLineLength);
        float avgDarkness = tools.findDarkestLine(pixels, context.getX(), context.getY(), minLineLength, nextLineLength, unlimitedTests ? -1 : lineTests, startAngle, deltaAngle, shading, context.getDstPosition());
